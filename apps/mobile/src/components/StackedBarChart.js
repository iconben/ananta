...
const BAR_WIDTH = 24;
const GAP = 6;
export function StackedBarChart({ bucketData, practices, maxVal }) {
    ...
    return (<View style={styles.container}>
      <Svg width={chartWidth} height={CHART_HEIGHT}>
        {bucketData.map((b, bi) => {
            ...
            practices.forEach((p) => {
                ...
                rects.push(<Rect key={p.id} x={x} y={CHART_HEIGHT - accumulated - segH} width={BAR_WIDTH} height={segH} fill={p.color} opacity={isLast ? 1 : 0.65} rx={0}/>);
                accumulated += segH;
            });
            return (<G key={bi}>
              {rects}
      ...
      <View style={[styles.labelsRow, { width: chartWidth }]}>
        {bucketData.map((b, bi) => {
            const isLast = bi === bucketData.length - 1;
            return (<View key={bi} style={[styles.labelWrap, { width: BAR_WIDTH + GAP }]}>
              <Text style={[
                    styles.label,
