...
export function Heatmap({ data, color }) {
    const weeks = useMemo(() => {
        const res = [];
    ...
    const bg = (v) => v === 0 ? 'rgba(255,255,255,.05)' :
        v < 500 ? `rgba(${rgb},.2)` :
            v < 5000 ? `rgba(${rgb},.45)` :
                v < 40000 ? `rgba(${rgb},.75)` :
                    `rgba(${rgb},1)`;
    return (<View style={{ overflowX: 'auto', paddingBottom: 4 }}>
      <View style={{ flexDirection: 'row', gap: 3 }}>
        {weeks.map((wk, wi) => (<View key={wi} style={{ flexDirection: 'column', gap: 3 }}>
            {wk.map((d, di) => (<View key={di} style={{
                    width: 11,
                    height: 11,
                    borderRadius: 2,
      ...
      <View style={{ flexDirection: 'row', gap: 5, marginTop: 6, alignItems: 'center' }}>
        ...
        {[.05, .2, .45, .75, 1].map((o, i) => (<View key={i} style={{
                width: 10,
                height: 10,
                borderRadius: 2,
