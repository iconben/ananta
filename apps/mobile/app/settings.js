...
const SECTION_TITLE = ({ children }) => (<Text style={styles.sectionTitle}>{children}</Text>);
const ROW = ({ label, last = false, children, }) => (<View style={[styles.row, !last && styles.rowBorder]}>
    <Text style={styles.rowLabel}>{label}</Text>
    {children}
  </View>);
const PLACEHOLDER_ROW = ({ label }) => (<View style={styles.placeholderRow}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Card>
          ...
          <View style={styles.fieldWrap}>
            ...
            <Input placeholder="你的法名或昵称…" value={user.name} onChangeText={(v) => handleUpdate('name', v)} maxLength={20}/>
          </View>

          {/* Bio */}
          <View style={styles.fieldWrap}>
            <Text style={styles.fieldLabel}>个人简介（可选）</Text>
            <Input placeholder="一句话介绍自己…" value={user.bio} onChangeText={(v) => handleUpdate('bio', v)} maxLength={100} multiline/>
          </View>
        </Card>
        ...
        <Card>
          ...
          <ROW label="修持数据是否公开">
            <Toggle value={user.dataPublic} onValueChange={(v) => handleUpdate('dataPublic', v)}/>
          </ROW>
          <ROW label="是否出现在排行榜">
            <Toggle value={user.inRanking} onValueChange={(v) => handleUpdate('inRanking', v)}/>
          </ROW>
          <ROW label="是否允许陌生人发送好友申请" last>
            <Toggle value={user.allowFriendReq} onValueChange={(v) => handleUpdate('allowFriendReq', v)}/>
          </ROW>
        </Card>
