...
export default function CampaignDetail() {
    ...
    const cRecords = useMemo(() => records.filter((r) => r.campaignId === id), [records, id]);
    ...
    const daysLeft = Math.ceil((new Date(campaign.end).getTime() - Date.now()) / 86400000);
    return (<View style={styles.container}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerInfo}>
            ...
            { backgroundColor: practice.color + '18', borderColor: practice.color + '35' },
              ...
              {done ? (<View style={[styles.practiceTag, { backgroundColor: 'rgba(74,222,128,.12)', borderColor: 'rgba(74,222,128,.3)' }]}>
                  <Text style={[styles.practiceTagText, { color: '#4ade80' }]}>✓ 圆满</Text>
                </View>) : (<View style={[styles.practiceTag, { backgroundColor: practice.color + '12', borderColor: practice.color + '28' }]}>
                  <Text style={[styles.practiceTagText, { color: practice.color }]}>
                    剩 {daysLeft} 天
                  </Text>
                </View>)}
          ...
          <TouchableOpacity style={styles.editBtn} onPress={() => setShowEditCampaign(true)}>
        ...
        <Card>
          ...
          {cRecords.length === 0 && (<Text style={styles.emptyText}>暂无记录</Text>)}
          {cRecords.map((r, i) => (<View key={r.id} style={[
                styles.recordRow,
      ...
      <CampaignSheet visible={showEditCampaign} onClose={() => setShowEditCampaign(false)} editCampaign={campaign}/>
    </View>);
}
