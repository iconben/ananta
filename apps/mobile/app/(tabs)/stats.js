...
export default function StatsTab() {
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.pillRow}>
        {PERIOD_LABELS.map(([v, l]) => (<Pill key={v} label={l} active={period === v} onPress={() => setPeriod(v)}/>))}
      </View>
      ...
      <Card>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <StackedBarChart bucketData={bucketData} practices={practices} maxVal={maxVal}/>
        </ScrollView>
        {/* Legend */}
        <View style={styles.legendRow}>
          {practices
            .filter((p) => Object.values(latestBucket?.totals ?? {}).some((v, i) => Object.keys(latestBucket?.totals ?? {})[i] === p.id && v > 0))
            .map((p) => (<View key={p.id} style={styles.legendItem}>
      ...
      <Card>
        ...
        <PracticeBreakdown practices={practices} latestBucket={latestBucket} prevBucket={prevBucket} maxVal={maxVal}/>
      </Card>
