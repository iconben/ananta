...
export function LogSheet({ visible, onClose, defaultPracticeId }) {
    ...
    const activeCampaigns = useMemo(() => campaigns.filter((c) => c.practiceId === practiceId &&
        !c.done &&
        new Date(c.end) >= new Date()), [campaigns, practiceId]);
    ...
    return (<Sheet visible={visible} onClose={onClose} title="记录功课">
      <View style={styles.field}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.practiceScroll}>
          <View style={styles.practiceRow}>
            {practices.map((p) => (<TouchableOpacity key={p.id} onPress={() => {
                setPracticeId(p.id);
                setCampaignId(null);
            }} style={[
                styles.practicePill,
      ...
      <View style={styles.field}>
        ...
        <Input value={count} onChangeText={setCount} placeholder="如：108" keyboardType="numeric" autoFocus/>
      </View>
      ...
      {activeCampaigns.length > 0 && (<View style={styles.field}>
          <Text style={styles.label}>计入发愿（可选）</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.practiceScroll}>
            <View style={styles.practiceRow}>
              <TouchableOpacity onPress={() => setCampaignId(null)} style={[
                styles.campaignPill,
              ...
              {activeCampaigns.map((c) => (<TouchableOpacity key={c.id} onPress={() => setCampaignId(c.id)} style={[
                    styles.campaignPill,
