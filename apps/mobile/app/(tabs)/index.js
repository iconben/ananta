...
export default function HomeTab() {
    ...
    const activeCampaigns = useMemo(() => campaignProgress.filter((c) => !c.done), [campaignProgress]);
    const doneCampaigns = useMemo(() => campaignProgress.filter((c) => c.done), [campaignProgress]);
    const getPractice = (id) => practices.find((p) => p.id === id);
    ...
    return (<View style={styles.container}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.toggleRow}>
          <Pill label="按发愿" active={viewMode === 'campaign'} color={COLORS.gold} onPress={() => setViewMode('campaign')}/>
          <Pill label="按课目" active={viewMode === 'practice'} color={COLORS.gold} onPress={() => setViewMode('practice')}/>
        </View>
        ...
        {viewMode === 'practice' ? (<>
            ...
            <Card style={styles.gridCard}>
              ...
              <View style={styles.practiceGrid}>
                {practiceStats.map((p) => (<TouchableOpacity key={p.id} style={[styles.practiceCard, { borderColor: p.color + '30' }]} onPress={() => router.push(`/practice/${p.id}`)} onLongPress={() => openEditPractice(p)}>
                    <TouchableOpacity style={styles.editBtn} onPress={() => openEditPractice(p)}>
                    ...
                    {p.today > 0 && (<Text style={[styles.todayBadge, { color: p.color }]}>
                        今日+{fmtN(p.today)}
                      </Text>)}
                  </TouchableOpacity>))}
              </View>
            ...
            <Card>
              ...
              {activeCampaigns.length === 0 && (<Text style={styles.emptyText}>暂无进行中的发愿</Text>)}
              {activeCampaigns.map((c) => {
                ...
                return (<TouchableOpacity key={c.id} style={styles.campaignRow} onPress={() => router.push(`/campaign/${c.id}`)} onLongPress={() => openEditCampaign(c)}>
            ...
            {/* Done campaigns */}
            {doneCampaigns.length > 0 && (<Card>
                <Text style={styles.sectionLabel}>✓ 已圆满的发愿</Text>
                {doneCampaigns.map((c) => {
                    const p = getPractice(c.practiceId);
                    if (!p)
                        return null;
                    return (<TouchableOpacity key={c.id} style={styles.campaignRow} onPress={() => router.push(`/campaign/${c.id}`)}>
      ...
      {/* FAB */}
      <TouchableOpacity style={styles.fab} onPress={() => setShowLogSheet(true)} activeOpacity={0.8}>
        <Text style={styles.fabIcon}>+</Text>
      </TouchableOpacity>

      {/* Sheets */}
      <PracticeSheet visible={showPracticeSheet} onClose={() => {
            setShowPracticeSheet(false);
            setEditPractice(null);
        }} editPractice={editPractice}/>

      <CampaignSheet visible={showCampaignSheet} onClose={() => {
            setShowCampaignSheet(false);
            setEditCampaign(null);
        }} editCampaign={editCampaign}/>

      <LogSheet visible={showLogSheet} onClose={() => setShowLogSheet(false)}/>
    </View>);
}
