...
export default function PracticeDetail() {
    ...
    const practiceRecords = useMemo(() => records.filter((r) => r.practiceId === id), [records, id]);
    const total = useMemo(() => practiceRecords.reduce((sum, r) => sum + r.count, 0), [practiceRecords]);
    ...
    return (<View style={styles.container}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity style={styles.editBtn} onPress={() => setShowEditPractice(true)}>
        ...
        {/* Related campaigns */}
        {relatedCampaigns.length > 0 && (<Card style={[styles.card, { borderColor: practice.color + '18' }]}>
            <Text style={styles.cardLabel}>🏮 相关发愿</Text>
            {relatedCampaigns.map((c) => {
                const pct = c.progress / c.goal;
                const daysLeft = Math.ceil((new Date(c.end).getTime() - Date.now()) / 86400000);
                return (<View key={c.id} style={styles.campaignRow}>
                  <Ring progress={pct} color={c.done ? '#4ade80' : practice.color} size={40}/>
                  <View style={styles.campaignInfo}>
                    <View style={styles.campaignHeader}>
                      <Text style={styles.campaignName}>{c.name}</Text>
                      {c.done ? (<Text style={styles.doneTag}>✓ 圆满</Text>) : (<Text style={styles.daysTag}>剩{daysLeft}天</Text>)}
                    </View>
                    ...
                    <ProgressBar progress={pct} color={c.done ? '#4ade80' : practice.color}/>
                  </View>
                  <TouchableOpacity style={styles.campaignEdit} onPress={() => handleOpenEditCampaign(c)}>
            ...
            {/* Merged total */}
            {relatedCampaigns.length > 1 && (<View style={styles.mergedTotal}>
                <View style={styles.mergedHeader}>
                  <Text style={styles.mergedLabel}>跨发愿合并</Text>
                  <Text style={[styles.mergedValue, { color: practice.color }]}>
                    {fmtN(relatedCampaigns.reduce((sum, c) => sum + c.progress, 0))}{' '}
                    /{' '}
                    {fmtN(relatedCampaigns.reduce((sum, c) => sum + c.goal, 0))}{' '}
                    {practice.unit}
                  </Text>
                </View>
                <ProgressBar progress={relatedCampaigns.reduce((sum, c) => sum + c.progress) /
                    relatedCampaigns.reduce((sum, c) => sum + c.goal)} color={practice.color}/>
              </View>)}
          </Card>)}
        ...
        <Card>
          ...
          {practiceRecords.length === 0 && (<Text style={styles.emptyText}>暂无记录</Text>)}
          {practiceRecords.map((r, i) => (<View key={r.id} style={[
                styles.recordRow,
      ...
      <PracticeSheet visible={showEditPractice} onClose={() => setShowEditPractice(false)} editPractice={practice}/>

      <CampaignSheet visible={showEditCampaign} onClose={() => setShowEditCampaign(false)} editCampaign={editCampaign}/>
    </View>);
}
